...
# Mantém seu downloader libcurl/libarchive.
...
# Otimização alvo: i7-3770 / Ivy Bridge.
# 32-bit continua sendo o padrão para compatibilidade com Half-Life/GoldSrc.
XASH_LTO="${XASH_LTO:-1}"             # 1 = ativa LTO
XASH_FAST_MATH="${XASH_FAST_MATH:-0}" # 1 = mais agressivo, pode alterar física/demos
XASH_PGO="${XASH_PGO:-off}"           # off | gen | use
XASH_PGO_DIR="${XASH_PGO_DIR:-$(pwd)/build/pgo-i73770}"

XASH_OPT_CFLAGS="-O3 -march=ivybridge -mtune=ivybridge -mfpmath=sse -pipe -fno-plt -fomit-frame-pointer -fstrict-aliasing -fno-semantic-interposition -fno-math-errno -fno-trapping-math -DNDEBUG -DXASH_I73770_ENGINE_PATCH=1"
XASH_OPT_LINKFLAGS="-Wl,-O1 -Wl,--as-needed -fno-plt"
...
if [ "$XASH_LTO" = "1" ]; then
  XASH_OPT_CFLAGS="$XASH_OPT_CFLAGS -flto"
  XASH_OPT_LINKFLAGS="$XASH_OPT_LINKFLAGS -flto"
fi
