...
/* CL_MODEL_OUTLINE_CVAR_GET_API_FIX: register cvars begin */
static void R_ModelOutline_RegisterCVars( void )
{
	if( !cl_model_outline )
		cl_model_outline = gEngfuncs.pfnCvar_Get( "cl_model_outline", "0", FFCVAR_ARCHIVE, "Enable stencil outline around visible player models" );

	if( !cl_model_outline_color )
		cl_model_outline_color = gEngfuncs.pfnCvar_Get( "cl_model_outline_color", "0 255 80", FFCVAR_ARCHIVE, "Outline color: R G B, accepts 0..255 or 0..1" );

	if( !cl_model_outline_thickness )
		cl_model_outline_thickness = gEngfuncs.pfnCvar_Get( "cl_model_outline_thickness", "4.0", FFCVAR_ARCHIVE, "Outline thickness/scale amount" );

	if( !cl_model_outline_brightness )
		cl_model_outline_brightness = gEngfuncs.pfnCvar_Get( "cl_model_outline_brightness", "1.0", FFCVAR_ARCHIVE, "Outline brightness/alpha" );

	if( !cl_model_outline_players_only )
		cl_model_outline_players_only = gEngfuncs.pfnCvar_Get( "cl_model_outline_players_only", "1", FFCVAR_ARCHIVE, "Only outline player studio models" );

	if( !cl_model_outline_depth_test )
		cl_model_outline_depth_test = gEngfuncs.pfnCvar_Get( "cl_model_outline_depth_test", "1", FFCVAR_ARCHIVE, "Keep depth test enabled so outline does not show through walls" );
}
/* CL_MODEL_OUTLINE_CVAR_GET_API_FIX: register cvars end */
