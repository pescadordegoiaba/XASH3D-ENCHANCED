...
CVAR_DEFINE_AUTO( cl_model_outline, "0", FCVAR_ARCHIVE, "Enable stencil outline around visible player models" );
CVAR_DEFINE_AUTO( cl_model_outline_color, "0 255 80", FCVAR_ARCHIVE, "Outline color: R G B, accepts 0..255 or 0..1" );
CVAR_DEFINE_AUTO( cl_model_outline_thickness, "4.0", FCVAR_ARCHIVE, "Outline thickness/scale amount" );
CVAR_DEFINE_AUTO( cl_model_outline_brightness, "1.0", FCVAR_ARCHIVE, "Outline brightness/alpha" );
CVAR_DEFINE_AUTO( cl_model_outline_players_only, "1", FCVAR_ARCHIVE, "Only outline player studio models" );
CVAR_DEFINE_AUTO( cl_model_outline_depth_test, "1", FCVAR_ARCHIVE, "Keep depth test enabled so outline does not show through walls" );
...
static void R_ModelOutline_RegisterOneCvar( convar_t *var )
{
	if( !var || !var->name || !var->name[0] )
		return;

	if( gEngfuncs.pfnGetCvarPointer( var->name, 0 ) )
		return;

	gEngfuncs.Cvar_RegisterVariable( var );
}

static void R_ModelOutline_RegisterCVars( void )
{
	R_ModelOutline_RegisterOneCvar( &cl_model_outline );
	R_ModelOutline_RegisterOneCvar( &cl_model_outline_color );
	R_ModelOutline_RegisterOneCvar( &cl_model_outline_thickness );
	R_ModelOutline_RegisterOneCvar( &cl_model_outline_brightness );
	R_ModelOutline_RegisterOneCvar( &cl_model_outline_players_only );
	R_ModelOutline_RegisterOneCvar( &cl_model_outline_depth_test );
}
...
static void R_ModelOutline_ParseColor( float *out_r, float *out_g, float *out_b, float *out_a )
{
	...
	if( cl_model_outline_color.string && cl_model_outline_color.string[0] )
	...
	brightness = cl_model_outline_brightness.value;
...
{
	if( cl_model_outline.value <= 0.0f )
		return 0;
	...
	/* Default: só modelos de jogadores. */
	if( cl_model_outline_players_only.value != 0.0f )
	{
...
static void R_StudioRenderModel( void )
{
	...
	thickness = cl_model_outline_thickness.value;
	thickness = R_ModelOutline_ClampFloat( thickness, 0.1f, 32.0f );
	scale = 1.0f + thickness * 0.010f;

	/*
	 * Passada 1:
	 * desenha o modelo normal e marca seus pixels no stencil como 1.
	 */
	...
	/*
	 * Passada 2:
	 * desenha o mesmo modelo ampliado, sólido, apenas onde stencil != 1.
	 * Com depth_test 1, o outline NÃO aparece através de paredes.
	 */
	...
	if( cl_model_outline_depth_test.value <= 0.0f )
		pglDisable( GL_DEPTH_TEST );
	else
		pglEnable( GL_DEPTH_TEST );
